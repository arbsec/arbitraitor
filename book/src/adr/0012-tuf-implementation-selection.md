...
AWS security bulletin 2026-019 disclosed multiple vulnerabilities in `tough` (the Rust TUF implementation) before version 0.22.0. This demonstrates that update clients are themselves high-risk parsers and authorization engines.
...
This provides signed updates, version rollback protection, expiration checking, and target integrity verification. It does NOT provide the full TUF delegation model. Full TUF is deferred until the ADR is finalized with conformance results.
