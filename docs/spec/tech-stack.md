...
> **Changelog vs v0.4:**
>
> - §1: added `arbitraitor-engine` row to the recommended baseline table (public embedding surface).
> - §3: workspace listing updated to match the actual 31-crate workspace (was 25 in v0.4); added `arbitraitor-engine` (name and boundary decided by ADR-0038, accepted), `arbitraitor-daemon`, `arbitraitor-mcp`, `arbitraitor-workspace-hack`, and `xtask`. Boundary rules extended to describe the pipeline engine as a first-class crate.
...
| CLI | `clap` derive |
| Public embedding API | Extracted `arbitraitor-engine` crate; pipeline engine consumed by CLI, daemon, MCP, and third parties (spec §40) |
...
arbitraitor/
  ...
  crates/
    ...
    arbitraitor-engine/          # pipeline engine (ADR-0038, accepted) — owns fetch → store → analyze → provenance → receipt → verdict → release
    arbitraitor-daemon/           # local Unix-socket daemon; thin consumer of arbitraitor-engine (socket I/O + queue + rate limit)
    arbitraitor-mcp/              # MCP server + AI-agent gateway; thin consumer of arbitraitor-engine
...
- `arbitraitor-engine` owns pipeline orchestration (fetch → store → analyze → provenance → receipt → verdict → release); it composes the I/O-producing crates above the `arbitraitor-core` state machine boundary, and exposes a single typed API consumed by `arbitraitor-cli`, `arbitraitor-daemon`, `arbitraitor-mcp`, and third-party embedders. Per spec §40.0, only this crate may transition the pipeline state machine across retrieval → release.
- `arbitraitor-cli` performs argument parsing, presentation, approval UX, and audit-event emission. It calls `arbitraitor-engine` and formats results; it does not compose pipeline stages itself (ADR-0027 trajectory redirected by spec §40.4).
- `arbitraitor-daemon` is a thin Unix-socket consumer of `arbitraitor-engine`; it owns socket I/O, the operation queue, capability-token verification, and rate-limiting only.
- `arbitraitor-mcp` is a thin JSON-RPC consumer of `arbitraitor-engine`; it owns MCP protocol handling, tool discovery, capability classes, and `AgentIdentity` audit attribution.
...
- The public API never exposes types from `arbitraitor-fetch`,
  ...
  or mapped to ID-stable structs in `arbitraitor-engine`;
- On `1.0`, breaking changes follow Rust RFC 1105: minor versions are additive, breaking changes require a major bump.

#### 3.5.3 Naming and boundary decision

ADR-0038 (accepted) resolved the open items here: the crate is named
`arbitraitor-engine` (per the reasoning in spec §40.7), and the API was
**extracted** out of `arbitraitor-daemon` into the new crate rather than
produced by a rename-and-split of the daemon.
...
Before publishing the `arbitraitor-engine` crate to crates.io for third-party
...
5. **ADR-0038 accepted.** Naming and extraction-vs-rename decision recorded;
